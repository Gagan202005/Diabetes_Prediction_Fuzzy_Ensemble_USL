...
## 2. Advanced Dataset Setup & Preprocessing
* **Outlier Capping (IQR Technique):** Instead of dropping extreme medical anomalies (e.g., highly abnormal BMI or glucose levels) which might skew standard scaler functions, an Interquartile Range (IQR) capping method was enforced mathematically. 
* **Feature Engineering:** Domain-specific combinations were formed (like consolidating `hypertension` and `heart_disease` into a single `health_risk_score`) to add richer context.
...
* **Advanced Encoding:** Categorical features (`gender`, `smoking_history`) were processed using **One-Hot Encoding** (creating binary dummy variables) rather than simple Label Encoding. This removes the problem of numerical/hierarchical bias implicit in arbitrary labels.
...
| :--- | :---: | :---: | :--- |
| **Logistic Regression** | `0.9592` | `0.56` | **Poor/Moderate**. The linear decision boundaries failed to capture the complex relationships, resulting in a high number of False Positives. |
| **Random Forest** | `0.9635` | `0.75` | **Good**. Decision trees handled the non-linear distributions well, providing a significant boost in classification accuracy. |
| **XGBoost** | `0.9739` | `0.79` | **Excellent**. Our strongest standalone learner. It aggressively minimized error residuals and perfectly balanced high precision with high recall. |
...
## 5. Conclusion
While the standalone XGBoost model performed equally to the Fuzzy Ensemble in raw F-1 metrics (F1 $0.79$), **the Fuzzy Ensemble provides a much more robust and explainable architecture.** 
